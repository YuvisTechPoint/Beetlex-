...
export const teamSetupSchema = z
  .object({
    teamMode: z.enum(['solo', 'create', 'join']),
...
export function createTrackSelectionSchema(teamMode: RegistrationTeamMode) {
  if (teamMode === 'join') {
...
export const registrationFormSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if ((data.teamMode === 'create' || data.teamMode === 'solo') && !data.trackId) {
      ctx.addIssue({
        code: 'custom',
